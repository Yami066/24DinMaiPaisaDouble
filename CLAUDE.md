...
24DinMaiPaisaDouble is a Python 3.12 CLI tool that automates four online workflows:
...
| Image gen | — | `gemini_image` (Gemini image API) |
| STT | `stt_provider` | `local_whisper`, `third_party_assemblyai` |

LLM always uses the local Ollama server. Image generation always uses Gemini image API.
...
- **Gemini image API** — for image generation
- **Go** — only needed for Outreach (Google Maps scraper)

## Contributing

PRs go against `main`. One feature/fix per PR. Open an issue first. Use `WIP` label for in-progress PRs. Check [docs/Roadmap.md](docs/Roadmap.md) for features.
