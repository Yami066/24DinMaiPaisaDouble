...
def main() -> int:
    ...
    api_key = cfg.get("gemini_image_api_key", "") or os.environ.get("GEMINI_API_KEY", "")
    gem_base = str(
        cfg.get(
    ...
    if api_key:
        ok("gemini_image_api_key is set")
    else:
        fail("gemini_image_api_key is empty (and GEMINI_API_KEY is not set)")
        failures += 1
 
    reachable, detail = check_url(gem_base, timeout=8)
    if not reachable:
        warn(f"Gemini image API base URL could not be reached: {detail}")
    else:
        ok(f"Gemini image API base URL reachable: {gem_base}")
