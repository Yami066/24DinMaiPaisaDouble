...
def get_gemini_image_api_base_url() -> str:
    """
    Gets the Gemini image API base URL.
...
def get_gemini_image_api_key() -> str:
    """
    Gets the Gemini image API key.

    Returns:
        key (str): API key
    """
    with open(os.path.join(ROOT_DIR, "config.json"), "r") as file:
        configured = json.load(file).get("gemini_image_api_key", "")
        return configured or os.environ.get("GEMINI_API_KEY", "")

def get_gemini_image_model() -> str:
    """
    Gets the Gemini image model name.

    Returns:
        model (str): Model name
    """
    with open(os.path.join(ROOT_DIR, "config.json"), "r") as file:
        return json.load(file).get("gemini_image_model", "gemini-3.1-flash-image-preview")

def get_gemini_image_aspect_ratio() -> str:
    """
    Gets the aspect ratio for Gemini image generation.

    Returns:
        ratio (str): Aspect ratio
    """
    with open(os.path.join(ROOT_DIR, "config.json"), "r") as file:
        return json.load(file).get("gemini_image_aspect_ratio", "9:16")
...
def get_image_provider() -> str:
    ...
    Valid values:
        "auto"         - tries Pollinations → Lexica → Picsum → gradient fallback
        "pollinations" - only Pollinations (+ gradient fallback)
        "lexica"       - only Lexica.art (+ gradient fallback)
        "picsum"       - only Picsum (+ gradient fallback)

    Returns:
        provider (str): The image provider key (default: "auto")
    """
    with open(os.path.join(ROOT_DIR, "config.json"), "r") as file:
        return json.load(file).get("image_provider", "auto")
